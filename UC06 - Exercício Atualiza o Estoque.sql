...
-- UC06 - Exercício Relatório Venda
...
-- UC06 - Exercício Relatório Vendas e Produtos.
...
-- UC06 - Exercício Atualiza o Estoque


SELECT * FROM produto;
UPDATE produto p
SET quantidade_estoque = (SELECT quantidade_estoque - SUM(iv.quantidade) FROM itens_venda iv WHERE iv.id_produto = p.id_produto AND iv.cancelado = FALSE) + 
(SELECT SUM(iv.quantidade) FROM itens_venda iv WHERE iv.id_produto = p.id_produto AND iv.cancelado = TRUE)
WHERE p.id_produto IN (SELECT id_produto FROM itens_venda GROUP BY id_produto);
