...
VALUES
 ...
 --   pj.status_
  ...
  --  INNER JOIN pessoa p ON p.id_pessoa = c.id_pessoa;
   ...
   -- UC06 - Relatório Projeto e Empreiteira
 ...
 --  o.data_inicio
  ...
  -- INNER JOIN pessoa pm ON pm.id_pessoa = cn.id_pessoa;
   ...
   -- UC06 - Relatório Empreiteira
   
   CREATE VIEW endereco AS
   SELECT 
    en.rua AS "rua empreiteira",
    en.cidade AS "cidade empreiteira",
    en.uf AS "uf empreiteira",
    en.cep AS "cep empreiteira",
	e.rua AS "rua mestre_obra",
    e.cidade AS "cidade mestre_obra",
    e.uf AS "uf mestre_obra",
    e.cep AS "cep mestre_obra",
    ende.rua AS "rua arquiteto",
    ende.cidade AS "cidade arquiteto",
    ende.uf AS "uf arquiteto",
    ende.cep AS "cep arquiteto"
    FROM obra o  
    INNER JOIN empreiteira ep ON ep.id_empreiteira = o.id_empreiteira
    INNER JOIN pessoa p ON p.id_pessoa = ep.id_pessoa
    INNER JOIN endereco en ON en.id_pessoa = p.id_pessoa
	INNER JOIN colaborador cn ON cn.id_colaborador = o.id_colaborador
    INNER JOIN pessoa pm ON pm.id_pessoa = cn.id_pessoa
    INNER JOIN endereco e ON e.id_pessoa = pm.id_pessoa
    INNER JOIN projeto pj ON pj.id_projeto = o.id_projeto
    INNER JOIN colaborador c ON c.id_colaborador = PJ.id_colaborador
    INNER JOIN pessoa pess ON pess.id_pessoa = c.id_pessoa
    INNER JOIN endereco ende ON ende.id_pessoa = pess.id_pessoa; 
